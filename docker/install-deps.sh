...
if [[ `uname` == 'Darwin' ]]; then
    ...
    if [[ $distribution == 'ubuntu' ]]; then
        ...
        if [[ $ubuntu_major_version -lt '12' ]]; then
            ...
            -E add-apt-repository -y ppa:chris-lea/zeromq
        elif [[ $ubuntu_major_version -lt '15' ]]; then # 14.xx
	    -E apt-get install -y software-properties-common
            -E add-apt-repository -y ppa:jtaylor/ipython
        else
	...
	if [[ $opensuse_major_version == '42.2' ]]; then
	    zypper install -y gcc curl cmake readline-devel IPython git-core \
        ...
        if [[ $fedora_major_version == '20' ]]; then
            ...
            dnf install -y make cmake curl readline-devel ncurses-devel \
                                gcc-c++ gcc-gfortran git gnuplot unzip \
                                libjpeg-turbo-devel libpng-devel \
                                ImageMagick GraphicsMagick-devel fftw-devel \
        ...
        if [[ $centos_major_version == '7' ]]; then
            ...
            yum install -y make cmake curl readline-devel ncurses-devel \
                                gcc-c++ gcc-gfortran git gnuplot unzip \
                                libjpeg-turbo-devel libpng-devel \
                                ImageMagick GraphicsMagick-devel fftw-devel \
                                sox-devel sox zeromq3-devel \
        ...
        yum --enablerepo=epel install -y zeromq3-devel
        pip install ipython
	...
	if [[ $debian_major_version == '8' ]] || [[ $debian_major_version == '9' ]]; then
	    ...
	    apt-get install -y libgraphicsmagick1-dev libfftw3-dev sox libsox-dev
